...
String SOS_NUM = "+60135272988";
String USER = "+60135272988";
String APN_NAME = "";
...
void loop()
{
...

  {
    ...
    if (Serial1.available())
    {
      ...
      {
        //check the state
        if (fromGSM == "SENDLOCATION\r" || fromGSM == "sendLocation\r" || fromGSM == "SendLocation\r" || fromGSM == "Sendlocation\r")
        {
          digitalWrite(A9G_LOWP, LOW);
          //////////google map link function////////
          Get_gmap_link_nocall();
          ////////////end google map link function//////////
          digitalWrite(A9G_LOWP, HIGH);// Sleep Mode ON
        }
    ...
    if (digitalRead(SOS_Button) == LOW && CALL_END == 1)
    {
      ...
      if (c == 5)
      {
        Get_gmap_link();  // Send Location with Call
    ...
    //////////////////////////////////////////////////////////CALLING USER/////////////////////////////
    else if(digitalRead(Call_Button) == LOW && CALL_END == 1)
    {
      Serial.print("Calling In.."); // Waiting for 5 sec
    for (c = 0; c < SOS_Time; c++)
      {
...
void Get_gmap_link()
{
  ...
  else
  {
    ...
    Serial1.println ("I am here" + Gmaps_link);
    delay(1000);
    Serial1.println((char)26);
    delay(1000);
...
void A9G_Ready_msg()
{
  ...
  Serial1.println ("AT YOUR SERVICE SIR");
  delay(1000);
  Serial1.println((char)26);
  delay(1000);
...
void Calling_User()
{
  ...
  Serial1.println("AT+CMGF=1");
  delay(1000);
  Serial1.println("AT+CMGS=\"" + USER + "\"\r");
  delay(1000);
  Serial1.println ("PICK UP THE CALLLLLLLL");
  delay(1000);
  Serial1.println((char)26);
  delay(1000);

  Serial.println("Calling Now");
  Serial1.println("ATD" + USER);
  if{
  CALL_END = 1 || CALL_DECLINED = 1; 
